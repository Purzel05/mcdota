...
import de.prentl.firsttestproject.entities.McdEntityType;
                    ...
                    for (McdMap.TowerLocation location: McdPlugin.towerLocations) {
                        ...
                        if (vec3DList != null) {
                            for (Vec3D spawnVec : vec3DList) {
                                ...
                                McdSkeleton skeleton = McdEntityType.skeletonType.spawn(spawnLocation);
                                if (skeleton != null) {
        ...
        for (McdMap.Side side: McdPlugin.sides) {
            for (McdMap.Lane lane: McdPlugin.lanes) {
                for (int i = 0; i < WAVES_SIZE; i++) {
                    ...
                    boolean doIt = true;
                    while (doIt) {
                        pigZombie = McdEntityType.pigZombieType.spawn(spawnLocation);
                        if (pigZombie.isBaby()) {
                            ...
                            Bukkit.getLogger().fine("Baby zombie removed ...");
                        } else {
                            doIt = false;
                        }
                    }
